...
bool Program::addParmOffset(string parmName, int offset, SymbolTable &localVars) // add the parameter to the local vars and attach the offset to the stack to it
{
...
bool Program::loadProg(ifstream& inFile, bool list) // load the program using inFile given. Return true if successful else false
{
    bool read = false;
    Commands cmd;
    Symbol s;
...
int Program::peek(string v, SymbolTable &localVars) // get a copy of the value of local variable (localVars points to correct place on stack)
{
    bool success=false;
    int result = 0;
...
void Program::push(string v, SymbolTable &localVars) // push local variable space onto stack and add variable to localVars
{
...
void Program::poke(string v, int val, SymbolTable &localVars) // set variable to value and use localVars to find place on stack
{
